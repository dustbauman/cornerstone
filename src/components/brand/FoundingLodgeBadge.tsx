...
type Variant = 'inline' | 'pill' | 'callout' | 'hero'
...
const VARIANT_CLASSES: Record<Variant, string> = {
  ...
  hero: 'inline-flex items-center gap-2 text-gold font-semibold text-sm uppercase tracking-[0.2em]',
}
...
export default function FoundingLodgeBadge({
  ...
  const markSize =
    variant === 'pill' ? 12 : variant === 'callout' || variant === 'hero' ? 18 : 14

  return (
    <span className={`${VARIANT_CLASSES[variant]} ${className}`}>
      <TyrianMark size={markSize} className="text-gold" />
      {variant === 'pill' ? (
        <span>{label}</span>
      ) : (
        <span
